...
# เก็บค่าสีไว้ใน list   S          Z          I       O        J        L       T
...
class Piece(object):
    def __init__(self, x, y, shape):
        ...
        self.rotation = 0 # การหมุนรูปร่าง
...
def convert_shape(shape):
    ...
    for i, line in enumerate(format):
        row = list(line) #sample data ['..0..']
        for j, column in enumerate(row):  # sample data ['.','.','0','.','.']
            if column == '0': # หาเลข 0
                positions.append((shape.x + j, shape.y + i)) #เพิ่มเข้า list pos
    # จัดดูปร่างให้อยู่ตรงกลาง
...
def empty_space(shape, grid):
    ...
    for pos in format:
        if pos not in accept_pos:
            if pos[1] > -1:             #check ไม่ให้ไม่ด้านข้าง
                return False
    return True
...
class play_zone():
    ...
    # sample data in lock position = ((x,y):(tomato))
    ...
    # เช็คจากข้างล่างขึ้นข้างบน
    # เมื่อลบแถวแล้วจะต้องเพิ่มแถวใหม่เข้าไป
    def check_tetris(grid, lock_pos):
        index = 0 # Pointer
        increases = 0 #จำนวนแถวที่นำออก
        for i in range(len(grid) - 1, -1, -1): #check row จากหลังมาด้านหน้า
            row = grid[i]
            if (0, 0, 0) not in row:
                increases += 1
                index = i
...
def main_game():
    ...
    while run:
        ...
        fall_time += clock.get_rawtime() #จะเช็คว่า while loop ทำงานนานเท่าไหร่
        clock.tick()
        ...
        if fall_time / 1000 >= fall_speed:
                fall_time = 0
                current_piece.y += 1
                if not (empty_space(current_piece, grid)) and current_piece.y > 0: #เช็คว่ารูปร่างที่ตกลงไปนั้นเป็น empty space รึป่าว
                    current_piece.y -= 1
                    change_piece = True
        ...
        if change_piece:
            for pos in shape_pos:
                p = (pos[0], pos[1])
                lock_position[p] = current_piece.color #เปลี่ยนสีของ Grid
                # sample data {(0,2):(lime)}
